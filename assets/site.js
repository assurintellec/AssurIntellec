...
function ensureInstagramEmbedScript() {
  ...
  instagramEmbedPromise = new Promise((resolve) => {
    const existing = document.querySelector('script[data-instagram-embed-script="true"]');
...
function renderInstagram(site) {
  ...
  if (!list.length) {
    holder.innerHTML = `
      <div class="instagram-placeholder">
        ...
        <span>${escapeHtml(section.empty_message || 'Add a public Instagram Reel URL in Decap CMS.')}</span>
      </div>
    `;
  ...
  /*
    IMPORTANT:
    Use Instagram's official embed block rather than forcing the
    /reel/.../embed iframe into a fixed-height iframe.

    This lets Instagram's own embed.js calculate its internal layout.
    The outer wrapper controls the visual 9:16 presentation.
  */
  holder.innerHTML = `
    <blockquote
