...
function loadGoogleFonts(theme) {
  ...
  const selected = [...new Set(families)]
    .filter(f => googleCandidates.has(f));
...
function applyTheme(site) {
  ...
  const map = {
    ...
    border: ['--border', 'value'],
    footer_background: ['--footer-bg', 'value'],
    footer_text: ['--footer-text', 'value'],
...
function renderHome(
  ...
  if (video) {
    ...
    video.autoplay =
      h.video_autoplay !== false;

    video.loop =
      h.video_loop !== false;

    heroAudioEnabled =
      h.video_audio_default_enabled !== false;
  ...
  setText(
    'servicesPreviewEyebrow',
    servicePreview.eyebrow
  );
  ...
  setText(
    ...
    servicePreview.description
  );

  setText(
    'servicesPreviewButton',
  ...
  setText(
    'whyPreviewDescription',
    whyPreview.description
  );
...
function renderAbout(site) {
  ...
  if (ph) {
    ...
    if (span) {
      ...
      span.textContent =
        about.profile_missing_title ||
        ui.profile_placeholder_title ||
    ...
    if (small) {
      ...
      small.textContent =
        about.profile_missing_help ||
        ui.profile_placeholder_help ||
        'Use Decap CMS → Site Settings → About Us';
    }
  }
...
function handleEnquirySubmit(
  ...
  if (
    ...
    finish(
      false,
      'Google Apps Script URL is not configured yet.'
    );
  }
}
...
function renderServiceDetail(
  ...
  setText(
    'serviceCaption1',
    service.gallery_caption_1 ||
    'Supporting visual'
  );

  setText(
    'serviceCaption2',
    service.gallery_caption_2 ||
    'Additional visual'
  );

  setText(
    'serviceImageText1',
    service.image_1_text ||
    service.image_1_alt ||
