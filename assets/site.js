...
function renderInstagram(site) {
    ...
    holder.innerHTML = `
      <div class="instagram-placeholder">
        ...
        <span>${escapeHtml(
          ...
          'Add a public Instagram Reel URL in Decap CMS.'
        )}</span>
      </div>
    `;
  ...
  /*
    IMPORTANT:
    Use a direct Instagram Reel /embed/ iframe instead of the Instagram
    JavaScript blockquote embed. The previous blockquote approach could
    leave an empty viewport when Instagram's embed.js did not hydrate.
  */
  const embedUrl =
  ...
  holder.innerHTML = `
    <iframe
      class="instagram-direct-iframe"
      src="${escapeHtml(embedUrl)}"
      title="${escapeHtml(item.title || 'Instagram Reel')}"
      loading="eager"
      scrolling="no"
      frameborder="0"
...
let enquiryPostFrame = null;
let enquiryPostFrameReady = false;

function getEnquiryPostFrame() {
  if (enquiryPostFrame && document.body.contains(enquiryPostFrame)) {
    return enquiryPostFrame;
  }

  enquiryPostFrame = document.createElement('iframe');
  enquiryPostFrame.name = 'assurIntellecEnquiryPostFrame';
  enquiryPostFrame.id = 'assurIntellecEnquiryPostFrame';
  enquiryPostFrame.title = 'AssurIntellec enquiry submission';
  enquiryPostFrame.setAttribute('aria-hidden', 'true');
  enquiryPostFrame.setAttribute('tabindex', '-1');
  enquiryPostFrame.style.position = 'fixed';
  enquiryPostFrame.style.left = '-10000px';
  enquiryPostFrame.style.top = '-10000px';
  enquiryPostFrame.style.width = '1px';
  enquiryPostFrame.style.height = '1px';
  enquiryPostFrame.style.border = '0';
  enquiryPostFrame.style.opacity = '0';
  enquiryPostFrame.style.pointerEvents = 'none';
  enquiryPostFrame.src = 'about:blank';

  document.body.appendChild(enquiryPostFrame);
  enquiryPostFrameReady = true;
...
function sendToGoogleAppsScript(
  ...
  if (navigator.onLine === false) {
    callback(
      false,
      'Your device appears to be offline. Please check the internet connection and try again.'
    );
    return;
  }

  const c = site.contact || {};
  const frame = getEnquiryPostFrame();

  if (!frame) {
    callback(false, 'Unable to prepare the enquiry service.');
    return;
  }
  ...
  /*
    The browser has accepted the normal cross-origin form POST.
    Do not wait for the Apps Script response before showing the customer
    confirmation. Apps Script may spend several seconds writing to the
    Sheet and sending email. The server records email status independently.
  */
  if (submitted) {
    window.setTimeout(() => {
      try {
...
function handleEnquirySubmit(event, site) {
  ...
  const finish = (ok, errorMessage) => {
    ...
    if (ok) {
      /*
        Do NOT reset the form. The customer can close the confirmation
        and then use Send on WhatsApp with the same entered details.
      */
    ...
    showToast(
      ...
      'Your enquiry could not be submitted. Please use WhatsApp or email.',
      false
    );
  };
