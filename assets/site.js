...
function renderInstagram(site) {
  ...
  /*
    Use Instagram's non-captioned embed endpoint and crop the external
    iframe at the parent viewport. The crop removes the profile/header,
    caption and lower embed UI while leaving the Reel media region visible.
  */
  const embedUrl =
    `https://www.instagram.com/reel/${encodeURIComponent(id)}/embed/?hidecaption=true`;

  holder.innerHTML = `
    <iframe
      id="instagramEmbedFrame"
      class="instagram-video-only-frame"
      src="${escapeHtml(embedUrl)}"
      title="${escapeHtml(item.title || 'Instagram Reel video')}"
      loading="eager"
  ...
  if (iframe) {
    ...
    iframe.addEventListener('error', () => {
      iframe.classList.add('embed-error');
    }, { once: true });
  }
}
...
function sendToGoogleAppsScript(url, data, site, callback) {
  ...
  const payload = new URLSearchParams({
    ...
    confirmation_email_enabled:
      c.customer_confirmation_email_enabled !== false ? 'true' : 'false',
